...
public:
  ...
  void damping_iter_no_odom_constraints(vector<IMUST> &x_stats, VOX_HESS &voxhess)
  {
  ...
  void damping_iter_diffusion(vector<IMUST> &x_stats, VOX_HESS &voxhess, vector<IMUST> &x_ring)
  {
      vector<int> updateIndex;
      for (int i = 0; i < x_stats.size(); i++)
      {
          for (int j = 0; j < x_ring.size(); j++)
          {
              if (x_stats[i].p == x_ring[j].p && x_stats[i].R == x_ring[j].R)
              {
      ...
      for (int i = 0; i < 10; i++)
      {
          ...
          //在这里把某些行列变为0
          //wlj add
          for (int idx = 0; idx < 6 * win_size; idx++)
          {
              if (std::find(updateIndex.begin(), updateIndex.end(), idx / 6) == updateIndex.end())
              {
                  // 将 Hess 中不在 updateIndex 中的行列置为0
                  Hess.block<6, 6>(idx, 0).setZero();
                  Hess.block<6, 6>(0, idx).setZero();
                  Hess.block<6, 6>(idx, idx).setZero();

                  // 将 JacT 中不在 updateIndex 中的值置为0
                  JacT.segment<6>(idx).setZero();
              }
          }
          ...
          // Update only the indices in updateIndex
          for (int idx : updateIndex)
          {
              int j = idx;
              Eigen::Matrix3d dR = Exp(dxi.block<3, 1>(DVEL * j, 0));
              x_stats_temp[j].R = dR * x_stats[j].R;
              x_stats_temp[j].p = dR * x_stats[j].p + dxi.block<3, 1>(DVEL * j + 3, 0);
              // Also update x_ring with the new values
              x_ring[j] = x_stats_temp[j];
          }
  ...
  void computeInitialHessMatrix(vector<IMUST> &x_stats, VOX_HESS &voxhess, Eigen::MatrixXd &Hess)
{
    vector<int> planes(x_stats.size(), 0);
    ...
    // 初始化D矩阵和JacT矩阵
    Eigen::VectorXd JacT(6 * win_size);
    ...
    // 初始化 Hess 矩阵的尺寸
    int hess_size = 6 * win_size; // 根据实际情况设置 Hess 矩阵的尺寸
    Hess = Eigen::MatrixXd::Zero(hess_size, hess_size);

    // 计算Hess矩阵
    residual1 = divide_thread_left(x_stats, voxhess, x_ab, Hess, JacT);

    // 打印Hess矩阵
    // std::cout << "Initial Hess matrix: \n" << Hess << std::endl;
}
...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT *> &feat_map, pcl::PointCloud<PointType> &pl_feat, const IMUST &x_key, int fnum)
{
  ...
  for (PointType &p_c : pl_feat.points)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      assert(voxel_size > 0.0 && "voxel_size 必须大于 0");
